...
wpierdol = ["R1O-GN", "LXQ2-T", "4-HWWF", "T5ZI-S","Y-MPWL","PR-8CA","J-ZYSZ","D-PNP9","G-M4GK","UGR-J2","MJ-5F9","Q-5211"]
jita = ["Jita"]
    ...
    try:
        for destynacjax in sysname:
            ...
            for x in r.json():
                ...
                eol = x['wormholeEol']
                if 0 < jmp < 14:
                    connectionlisttemp.append(f' {destynacjax} \n sygnatura: {sig} \n ile: {jmp} \n wlotowy: {name} \n region: {reg} \n EoL: {eol}')
                if jmp > 14:
                    daleko = (f'{destynacjax} za daleko albo nima')
            if not connectionlisttemp:
                connectionlist.append(daleko)
